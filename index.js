...
var hostname_artemis = 'localhost',
  path_artemis = '/artemis-web/debug/',
  ssl = false,
  hostname_data = 'https://s.enygma.id/artemis/'
var gmt_here = '+07:00'
  ...
  base64ToFile(str) {
    if (this.base64CheckType(str)) {
      ...
      return {
        ...
        file_blob: new File(
          [
            new Blob([new Buffer.from(str.split(',')[1], 'base64')], {
...
class Utilities {
  ...
  async OpenApiArtemis(method, path, data) {
    try {
      ...
      return await this.httpsPost(hostname_artemis, path_artemis, options, ssl)
    } catch (error) {
...
async function Runner() {
  ...
  let vehicleType = [
      { id: 0, descrition: 'others' },
      { id: 1, descrition: 'passenger vehicle' },
      { id: 2, descrition: 'truck' },
      { id: 3, descrition: 'sedan' },
      { id: 4, descrition: 'minivan' },
      { id: 5, descrition: 'light truck' },
      { id: 6, descrition: 'pedestrian' },
      { id: 7, descrition: 'two wheeler' },
      { id: 8, descrition: 'tricycle' },
      { id: 9, descrition: 'suv/mpv' },
      { id: 10, descrition: 'middle-sized bus' },
      { id: 11, descrition: 'motor vehicle' },
      { id: 12, descrition: 'non motor vehicle' },
      { id: 13, descrition: 'small sedan' },
      { id: 14, descrition: 'mini sedan' },
      { id: 15, descrition: 'pickup truck' },
      { id: 16, descrition: 'container truck' },
      { id: 17, descrition: 'minitruck and dropside trailer' },
      { id: 18, descrition: 'dump truck' },
      { id: 19, descrition: 'crane and engineering vehicle' },
      { id: 20, descrition: 'oil tank truck' },
      { id: 21, descrition: 'concrete mixer' },
      { id: 22, descrition: 'flatbed tow truck' },
      { id: 23, descrition: 'hatchback' },
      { id: 24, descrition: 'saloon' },
      { id: 25, descrition: 'sport sedan' },
      { id: 26, descrition: 'minibus' },
    ],
    vehicleColor = [
      { id: 0, descrition: 'other color' },
      { id: 1, descrition: 'white' },
      { id: 2, descrition: 'silver' },
      { id: 3, descrition: 'grey' },
      { id: 4, descrition: 'black' },
      { id: 5, descrition: 'red' },
      { id: 6, descrition: 'dark-blue' },
      { id: 7, descrition: 'blue' },
      { id: 8, descrition: 'yellow' },
      { id: 9, descrition: 'green' },
      { id: 10, descrition: 'brown' },
      { id: 11, descrition: 'pink' },
      { id: 12, descrition: 'purple' },
    ]
  ...
  const job = new CronJob('0,5,10,15,20,25,30,35,40,45,50,55 * * * *', async () => {
    ...
    let data_cross_record = await utility.ProcessCrossRecords(
      fiveminsbefore.format_tz,
      now.format_tz
    )
    if (data_cross_record.response.data.total > 0) {
      ...
      for (const [i, x] of data_result_anpr.entries()) {
        ...
        data_result_anpr[i].vehicleColor_text = vehicleColor.find(
          (v) => v.id == x['vehicleColor ']
        )?.descrition
        data_result_anpr[i].vehicleType_text = vehicleType.find(
          (v) => v.id == x['vehicleType ']
        )?.descrition
      }
